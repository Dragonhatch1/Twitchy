...
public class PlayerFlipRenderer {
    ...
    public void onRenderPre(RenderPlayerEvent.Pre event) {
        ...
        float x = (float) (event.entityPlayer.lastTickPosX
            + (event.entityPlayer.posX - event.entityPlayer.lastTickPosX) * partialTicks
            - RenderManager.renderPosX);
        float y = (float) (event.entityPlayer.lastTickPosY
            + (event.entityPlayer.posY - event.entityPlayer.lastTickPosY) * partialTicks
            - RenderManager.renderPosY);
        float z = (float) (event.entityPlayer.lastTickPosZ
            + (event.entityPlayer.posZ - event.entityPlayer.lastTickPosZ) * partialTicks
        ...
        GL11.glTranslatef(-x, -y, -z); // undo our own translate, so the real translate that runs next composes
                                       // correctly
    }
