...
public final class RaidManager {
    ...
    public static void handleRaid(RaidEvent event) {
        ...
        Twitchy.LOG.info(
            ...
            event.from_broadcaster_user_name, event.viewer_count, regularCount, bossCount);
        ...
        TwitchApiClient.sendShoutout(
                TwitchSessionManager.INSTANCE.credentials(), event.from_broadcaster_user_id)
            .exceptionally(ex -> {
                Twitchy.LOG.warn("Failed to send shoutout for {}: {}", event.from_broadcaster_user_name, ex.getMessage());
                return null;
            });
    }
